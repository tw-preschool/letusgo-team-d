...
describe "register page", :type =>:feature do
  ...
  context "register test", :js => true do
    ...
    it "should raise error when input a wrong email",focus: false do
      visit '/user/register'
      fill_in 'register-username', :with => 'd.com'
      expect(page).to have_content("请输入正确的email")
    end
    it "should raise error when input a wrong password",focus: false do
      visit '/user/register'
      fill_in 'register-password', :with => '111'
      expect(page).to have_content("长度不得小于6个字符")
    end
    it "should raise error when password and confirmPassword are different",focus: false do
      visit '/user/register'
      fill_in 'register-password', :with => '111222'
      fill_in 'register-confirmPassword', :with => '111111'
      expect(page).to have_content("两次密码应一致")
    end
    it "should raise error when input a wrong name",focus: false do
      visit '/user/register'
      fill_in 'register-name', :with => '#'
      expect(page).to have_content("姓名中不可包含数字或下划线或特殊字符")
    end
    it "should raise error when input a wrong telephone number",focus: false do
      visit '/user/register'
